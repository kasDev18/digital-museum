...
- **`/detail/[id]` renders the full Epic 5 layout** (`DetailPageContent`, Stories 5.1-5.4 — see the Detail Page section below) rather than Story 3.5's original placeholder. An unknown id still falls through to the site's own chrome-complete `not-found.tsx`.
...
### Detail Page — Layout, Media Carousel, Audio & Zoom (Epic 5, Stories 5.1-5.4, stretch scope)
...
- **`MediaCarousel`** (`src/app/detail/components/media-carousel`, Stories 5.2/5.4): a single active slide inside an `overflow: hidden` frame, advanced via a CSS `transform`-driven track. Four independent ways to change slides — the "More Images" button (bottom-right, per the mock's downward-chevron treatment), clickable dot indicators (also the current-slide indicator), `ArrowLeft`/`ArrowRight` keys while focused, and touch swipe — all wrapping at both ends. Single-item media (e.g. the Bamboo Pen) hides the "More Images" button and dots entirely rather than showing controls with nothing to advance to. A failed image load (see the unbacked-assets note above) hides behind the frame's own background instead of a broken-image icon, matching `ArtifactThumbnail`'s existing `onError` convention. **Zoom in/out (Story 5.4):** a top-right icon-button overlay applies a CSS `transform: scale(...)` (0.5x-3x, 0.5x steps) to the active slide's image only, natively disabling each button at its own boundary; resets to 1x on every slide change via all four navigation paths above, and stays visible even for single-image artifacts (unlike the "More Images"/dots controls, which have nothing to gate zoom on).
- **`AudioPlayer`** (`src/app/detail/components/audio-player`, Stories 5.1/5.3): native HTML5 `<audio>`, a play/pause toggle, a real seekable `<input type="range">` progress bar layered with a decorative tick-mark waveform visualization behind it, `m:ss / m:ss` elapsed/total labels, and a mute/unmute toggle. `audioUrl: null` renders a visually-present but disabled state (play/pause and mute both) rather than being omitted. The waveform's "played" tick count is purely a stylized readout of `currentTime`/`duration`, not a real analysis of the audio signal — the range input underneath (made visually transparent) is still the one real interactive element handling drag/click/keyboard seeking. Per the Mshatta Façade detail mock, this player is a fixed white pill with navy icons/text (`bg-white`/`text-navy`/`bg-navy`) regardless of the site's own light/dark theme — a deliberate "white card" design element, not a themed surface.
- See `_bmad-output/implementation-artifacts/deferred-work.md` for the full list of known, accepted gaps from this pass (the still-unbacked media/audio assets, pan-while-zoomed/pinch-to-zoom left as optional per Story 5.4's own Technical Notes, and the "Next story" ignoring an active List page category filter by design).
